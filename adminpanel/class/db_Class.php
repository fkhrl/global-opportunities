...
class db_class {
    ...
    public function open() {
        $con=mysqli_connect("localhost", "root", "", "global_new");
        return $con;
    }

    public function pdocon() {
        $db=new PDO("mysql:host=localhost;dbname=global_new;", "root", "");
    ...
    function baseUrl($suffix='') {
        ...
        $web_root=$protocol . $_SERVER['HTTP_HOST'] . "/" . "global/global/adminpanel/";
        $suffix=ltrim($suffix, '/');
    ...
    function LbaseUrl($suffix='') {
        $protocol=strpos($_SERVER['SERVER_SIGNATURE'], '443') !== false ? 'https://' : 'http://';
        $web_root=$protocol . $_SERVER['HTTP_HOST'] . "/" . "global/global/";
        $suffix=ltrim($suffix, '/');
        return $web_root . trim($suffix);
    }
